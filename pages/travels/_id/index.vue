            ...
            <v-container>
                <v-layout row wrap align-center>
                    ...
                    <v-layout column justify-center align-center>
                        ...
                        <v-flex xs12 md3>
                            <h1 style="font-size: 1em"> {{ source }} - {{ destination}} </h1>
                        </v-flex>
                        <v-flex xs12 md3>
                            <h1 style="font-size: 1em"> {{ cls }} </h1>
                        </v-flex>
                        <v-flex xs12 md3>
                            <h1 style="font-size: 1em"> {{ price }} </h1>
                        </v-flex>
                    </v-layout>
                    </v-flex>
              <v-flex xs12 md6>
...
export default {
  ...
  asyncData(context) {
      return new Promise((resolve, reject) => {
          var url = 'http://localhost:3000/fetchTravels/' + context.params.id;
          console.log(url);
          axios.get(url).then(res => {
              console.log(res.data);
              this.cls = res.data.class;
              resolve(res.data)
          }).catch(err => {
              reject(err)
          })
